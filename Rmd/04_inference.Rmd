...
$$
t_{k}:=\frac{\hat \beta_{k} - c}{\sigma (\hat \beta_{k})}
$$
...
Consider the testing problem $\boldsymbol R' \beta = r$, where $\boldsymbol R  \in \mathbb R^{p+K}$ is a pre-specified set of linear combinations and $r \in \mathbb R^p$  is a restriction vector. 
...
$$
\frac{\left(S S R_{R}-S S R_{U}\right) / p}{S S R_{U} /(n-K)}
$$
